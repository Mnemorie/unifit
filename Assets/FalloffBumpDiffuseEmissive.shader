...
////// Emissive:
                ...
                float node_4116 = 0.0;
                float node_7856 = 1.0;
                float node_2501 = (node_7856 + ( (distance(objPos.rgb,i.posWorld.rgb) - node_4116) * (node_4116 - node_7856) ) / (_DistanceFalloff - node_4116));
                float node_6470 = min(_RangeMin,_RangeMax);
                float node_399 = 0.0;
                float node_7501 = saturate((node_399 + ( (((saturate((node_1126 + ( (_Distance_var.r - node_1122) * ((node_5906+node_1127) - node_1126) ) / (1.0 - node_1122)))*_Emission_var.g*node_2501)+_Emission_var.r) - node_6470) * (1.0 - node_399) ) / (max(_RangeMin,_RangeMax) - node_6470)));
                float node_487 = 2.0;
                float3 node_1287 = lerp((_EmissionColor.rgb*node_7501*node_487),(1.0 - (node_487*(1.0 - _EmissionColor.rgb)*(1.0 - node_7501))),node_7501);
                float3 node_1055 = (node_1287+(pow(max((node_1287+(-1.0)),0.01),2.0)*100.0)); // Add HDR to Color
                float3 emissive = node_1055;
/// Final Color:
