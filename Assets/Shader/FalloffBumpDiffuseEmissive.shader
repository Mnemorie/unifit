...
Shader "Shader Forge/FalloffBumpDiffuseSpecularEmissive" {
    Properties {
        ...
        _DistanceRange ("Distance Range", Range(0, 1)) = 0.1
...
///////// Gloss:
                ...
                float node_710 = dot((_Specular_var.rgb*(_Gloss*2.0)),float3(0.3,0.59,0.11)); // Specular Gloss
                float gloss = node_710;
...
////// Emissive:
                ...
                float node_1122 = 0.0;
                float node_5906 = (_DistanceRange*1.25);
                float node_1127 = 0.5;
                float node_1126 = (node_5906-node_1127);
                float4 _Emission_var = tex2D(_Emission,TRANSFORM_TEX(i.uv0, _Emission));
                float node_6470 = min(_RangeMin,_RangeMax);
                float node_399 = 0.0;
                float node_7501 = saturate((node_399 + ( (((saturate((node_1126 + ( (_Distance_var.r - node_1122) * ((node_5906+node_1127) - node_1126) ) / (1.0 - node_1122)))*_Emission_var.g)+_Emission_var.r) - node_6470) * (1.0 - node_399) ) / (max(_RangeMin,_RangeMax) - node_6470)));
                float node_487 = 2.0;
